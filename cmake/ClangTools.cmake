...
if(CLANG_TIDY_PROG AND NOT TARGET tidy)
  if(NOT CMAKE_EXPORT_COMPILE_COMMANDS)
    message(
      WARNING "Invoke Catkin/CMake with '-DCMAKE_EXPORT_COMPILE_COMMANDS=ON'
...
if(PARALLEL_PROG)
  ...
  message(
    WARNING
      "parallel is not installeld. To enable parallelization of clang-tidy, try with 'sudo apt install parallel'."
  )
  set(PARALLEL_PROG "")
  set(PARALLEL_DELIMITER "")
endif()
...
function(add_format_target _target)
  ...
  add_custom_target(
    check-format-${_target}
    COMMAND
      ${CLANG_FORMAT_PROG} -output-replacements-xml ${ARG_FILES} | grep
      "<replacement " > /tmp/log && exit 1 || exit 0
    WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}/..
...
function(add_tidy_target _target)
  ...
  add_custom_target(
    check-tidy-${_target}
    COMMAND
      ${PARALLEL_PROG} ${CLANG_TIDY_PROG} -p=${CMAKE_BINARY_DIR}
      ${PARALLEL_DELIMITER} ${ARG_FILES} | grep . && exit 1 || exit 0
    WORKING_DIRECTORY ${CMAKE_CURRENT_LIST_DIR}/..
    DEPENDS ${ARG_DEPENDS}
    COMMENT "Running clang-tidy for ${_target}"
    VERBATIM)
